...
export function AIAssistant() {
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      const aiMsg: Message = { role: 'assistant', content: data.reply, intent: data.intent, id: `a_${Date.now()}` }
      setMessages(prev => [...prev, aiMsg])

      // Show hire banner when hiring intent detected
      if (data.intent === 'hire') {
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 sm:right-6 z-50 w-[calc(100vw-2rem)] sm:w-[400px] max-h-[580px] bg-background border border-border rounded-2xl shadow-2xl overflow-hidden flex flex-col"
          >
            {/* ── Header ── */}
            <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-blue-600 to-cyan-500 flex-shrink-0">
              ...
              <div>
                <p className="font-semibold text-white text-sm">{settings.chatbot_name || 'Portfolio AI'}</p>
                <p className="text-white/70 text-xs">Ask about Abhishek's work</p>
              </div>
              <div className="ml-auto flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                <span className="text-white/70 text-xs">Online</span>
                {messages.length > 1 && (
                  <button onClick={clearChat} className="ml-1 text-white/50 hover:text-white/90 text-xs transition-colors" title="Clear chat">
                    ↺
                  </button>
                )}
              </div>
            ...
            <AnimatePresence>
              {showHireBanner && (
                <motion.div
                  ...
                  className="bg-gradient-to-r from-blue-600/20 to-cyan-500/10 border-b border-blue-500/20 overflow-hidden"
                >
                  <div className="flex items-center gap-3 px-4 py-2.5">
                    <Briefcase className="w-4 h-4 text-blue-400 flex-shrink-0" />
                    <p className="text-xs text-blue-300 flex-1">
                      <strong>Ready to hire Abhishek?</strong> He's available for new opportunities!
                    </p>
                    ...
                    <button onClick={() => setShowHireBanner(false)} className="text-white/40 hover:text-white/70 ml-1">
            ...
            <div className="px-3 py-3 border-t border-border flex-shrink-0">
              <div className="flex gap-2 items-center">
                ...
                <input
                  ...
                  className="flex-1 bg-secondary rounded-full px-4 py-2 text-sm outline-none placeholder-muted-foreground text-foreground"
                />
                <button
                  onClick={() => send()}
