...
async function getJourneyStats() {
  try {
    const blogs = await sql`SELECT COUNT(*) as count FROM journey_blogs`
    const certs  = await sql`SELECT COUNT(*) as count FROM journey_certificates`
    return {
      blogCount: parseInt(blogs[0]?.count || '0'),
      certCount: parseInt(certs[0]?.count  || '0'),
    }
  } catch { return { blogCount: 0, certCount: 0 } }
}
